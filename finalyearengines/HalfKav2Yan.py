...
class LayerStacks(nn.Module):
    ...
    def forward(self, x, layer_stack_indices=None):
        ...
        l1x_ = self.shift_and_clamp(l1c_)
        l1y_ = self.relu(l1x_)
        ...
        l2x_ = self.shift_and_clamp(l2c_)
        l2y_ = self.relu(l2x_)
...
class MyNNUE(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, accum, psqt_indices=None, layer_stack_indices=None):
        # 1) Split
        ...
        main_accum_int = main_accum.to(torch.int32)
        main_accum_int >>= 6   # SHIFT=6
        main_accum_int = torch.clamp(main_accum_int, 0, 127)
        main_accum = main_accum_int.to(torch.float32)
        # # ---- SHIFT & CLAMP EXAMPLE ----
        ...
        psqt_indices = torch.full(
            (main_accum.shape[0],),
            psqt_indices,
            dtype=torch.long,
            device=main_accum.device
        )
